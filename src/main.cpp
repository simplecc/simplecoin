...
uint256 hashGenesisBlock("0x818d49c61674d1cf17c54138816cde042201b4459d545ce161cfa49ef5234236");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Simplecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Simplecoin Signed Message:\n";
...
{
    // Simplecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 9000; // Simplecoin: 2.5 hours
static const int64 nTargetSpacing = 90; // Simplecoin: 1.5 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Simplecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Simplecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Simplecoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x982a215e3ad65710b6db8fbdbda190d8f121901c6c86a12b05c9053eba14e304");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "28 january 2014";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1390947630;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 386566018;

        if (fTestNet)
        {
            block.nTime    = 1390937016;
            block.nNonce   = 386154641;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xf5fd86607614dad0f090b7bb4d80548dce53e8d70c8989123a49dfa168d3ab80"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Simplecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SimplecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SimplecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SimplecoinMiner(CWallet *pwallet)
{
    printf("SimplecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("simplecoin-miner");
    ...
    try { loop {
        ...
        printf("Running SimplecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SimplecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SimplecoinMiner, pwallet));
}
